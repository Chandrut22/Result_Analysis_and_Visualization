...
def upload(request):
    if request.method == 'POST':
        ...
        at = []
        pdf_path = os.path.join('media/', pdf_file.name) # update path here
        ...
        for i in range(str_page, end_page + 1):
            df = read_pdf(BytesIO(pdf_content), pages=i)[0]
            col = list(df.columns)
        ...
        replace = {'O': 10, 'UA': 0, 'A+': 9, 'A': 8, 'B+': 7, 'B': 6, 'C': 5, 'U': 0}
        for i in replace.keys():
            data.replace(i, replace[i], inplace=True)
        data.rename(columns={data.columns[0]: 'Reg.Number', data.columns[1]: 'Stud.Name'}, inplace=True)
        data['Reg.Number'] = data['Reg.Number'].apply(pd.to_numeric)
        data.to_csv('media/original_data/Student_Result.csv', encoding='utf-8', index=False)
        data.to_excel('media/original_data/Student_Result.xlsx', index=False)
        ...
        print(sub_name)
        while columns_with_nan != [] or 'Grade' in sub_name:
            print('PDF is not in the format')
...
def upload_excel(request):
    if request.method == 'POST':
        ...
        excel_path = os.path.join('media/', excel_file.name) 
...
def overall_per(request):
    data = pd.read_csv('media/original_data/Student_Result.csv')
    df = data.copy()
...
def gpa_performance(request):
    data = pd.read_csv('E:\sra\media\Student_Result.csv')
